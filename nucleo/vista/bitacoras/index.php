<?php
	...
	if(isset($_GET)){
		if(isset($_GET['p'])){
			...
			switch ($lcPagina) {
				case "listabitacoras":
					$lcPagina="listaBitacoras";
					break;
					
				case "registrobitacoras":
					$lcPagina="registroBitacoras";
					break;					
			}
	...
	if($llPagina==false){
		...
		$loBitacoras = new NUCLEO\Bitacoras();
		$lcTipoMenu = 'LISTADO'; // LISTADO / ICONOS
		$laOpcionesMenu = [];
		
		foreach($loBitacoras->getTiposBitacoras() as $laTipoBitacora){
			$laOpcionesMenu[$laTipoBitacora['CODIGO']] = [
				'url' => 'modulo-bitacoras&p=listaBitacoras&q='.$laTipoBitacora['CODIGO'],
				'icono' => (empty($laTipoBitacora['ICONO'])?$laTipoBitacora['ICONO']:'book-medical'),
				'titulo' => $laTipoBitacora['TITULO'],
				'texto' => $laTipoBitacora['DESCRIPCION'],
			];
		}
	...
	<div class="card mt-3">
		<div class="card-header">
			<ul class="nav nav-tabs  card-header-tabs" id="hcwTab" role="tablist">
				<li class="nav-item">
					<a class="nav-link active" id="alerta-tab" data-toggle="tab" href="#alerta" role="tab" aria-controls="menu" aria-selected="true">Bit&aacute;coras</a>
				</li>
			</ul>
		</div>
