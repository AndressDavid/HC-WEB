<?php
/* ********************  GENERACIÓN DE XML DE FE Y DS PARA FACTURE  ******************** */
namespace NUCLEO;


class FeGenerarXMLFacture
{
	/* Tipo de documento */
	protected $cTipoFac = '';
	public $cKeyControl = '';
	...
	public $oEmisor = null;
	public $oEmisorFA = null;
	public $oEmisorDS = null;
	...
	public function __construct($taConfig, $tcSLD = '')
	{
		...
		$this->oEmisorFA = $this->datosEmisor();
		$this->oEmisorDS = $this->datosEmisorDS();
	}
	 ...
	 */
	public function obligacionesNit($tcNit)
	{
		$lcRetorna = '';
		$laObliga = $this->oDb
	 ...
	 */
	public function nodoCabecera($taData)
	{
	 ...
	 */
	public function nodoNumeracionDIAN($tnFactura, $tcTipoRes='RESOLFAC')
	{
		global $goDb;
	 ...
	 */
	public function nodoReferenciasNotas($taData)
	{
	 ...
	 */
	public function nodoFacturasRelacionadas($taData)
	{
		...
		foreach($taData as $laFactura) {
			$loFacturaRelacionada = $this->oDomDoc->createElement('FacturaRelacionada');
				...
				$loFacturaRelacionada->setAttributeNode(new \DOMAttr('CUFE', trim($laFactura['cufe'])));
	 ...
	 */
	public function nodoNotificacion($tcListaCorreos)
	{
		$loReturn = false;
	 ...
	 */
	public function nodoCliente($taData, $tbNoEsDS=true)
	{
	 ...
	 */
	public function nodoMediosDePago($taData)
	{
	 ...
	 */
	public function nodoImpuestos($taData)
	{
	 ...
	 *			porcen:	Porcentaje
	 */
	public function nodoRetenciones($taData)
	{
		...
		foreach($taData as $laRetencion) {
			$loRetencion = $this->oDomDoc->createElement('Retencion');
				...
				$loSubtotal = $this->oDomDoc->createElement('Subtotal');
					...
					$loSubtotal->setAttributeNode(new \DOMAttr('Porcentaje', number_format($laRetencion['porcen'],5,'.','')));
	 ...
	 */
	public function nodoAnticipos($taData)
	{
	 ...
	 *			vrbase: Valor base
	 */
	public function nodoDescuentoOCargo($taData)
	{
	 ...
	 */
	public function nodoTotales($taData)
	{
	 ...
	 */
	public function nodoLinea($taData)
	{
		$loReturn = $this->oDomDoc->createElement('Linea');
			...
			if (in_array($this->cTipDocXml, ['DS','NS'])) {
	 ...
	 */
	public function nodoExtensiones($taData, $taDataSalud=false, $tcDatExt='extensiones')
	{
		...
		// Interoperabilidad
		if (is_array($taDataSalud) && $this->aCnf['Salud']['enviar']) {
	...
	/*
	 *	Organiza la información del emisor en un nodo
	 */
	public function datosEmisor()
	{
		// $loDomDoc = new \DOMDocument($this->aCnf['versionXml'], $this->aCnf['encodingXml']);
		$loEmisor = $this->oDomDoc->createElement('Emisor');

		// Genera nodo con datos del emisor
		foreach ($this->aCnf['emisor'] as $lcClave => $lcValor) {
			$loEmisor->setAttributeNode(new \DOMAttr($lcClave, $lcValor));
		}

		if (count($this->aCnf['ciiu'])>0) {
			$loCodigosCIIU = $this->oDomDoc->createElement('CodigosCIIU');
			foreach ($this->aCnf['ciiu'] as $laCIIU) {
				$loCIIU = $this->oDomDoc->createElement('CIIU');
				$loCIIU->nodeValue = $laCIIU;
				$loCodigosCIIU->appendChild($loCIIU);
				unset($loCIIU);
			}
			$loEmisor->appendChild($loCodigosCIIU);
			unset($loCodigosCIIU);
		}

		$loDireccion = $this->oDomDoc->createElement('Direccion');
		foreach ($this->aCnf['direccion'] as $lcClave => $lcValor) {
			$loDireccion->setAttributeNode(new \DOMAttr($lcClave, $lcValor));
		}
		$loEmisor->appendChild($loDireccion);
		unset($loDireccion);

		if (count($this->aCnf['direccionAdicional'])>0) {
			$loAdicionales = $this->oDomDoc->createElement('DireccionesAdicionales');
			foreach ($this->aCnf['direccion'] as $lcValor) {
				$loDireccion = $this->oDomDoc->createElement('Direccion');
				$loDireccion->nodeValue = $lcValor;
				$loAdicionales->appendChild($loDireccion);
				unset($loDireccion);
			}
			$loEmisor->appendChild($loAdicionales);
			unset($loAdicionales);
		}

		$loObligacionesEmisor = $this->oDomDoc->createElement('ObligacionesEmisor');
		foreach ($this->aCnf['obligaciones'] as $lcObligacion=>$lcDescrOblig) {
			$loCodigoObligacion = $this->oDomDoc->createElement('CodigoObligacion');
			$loCodigoObligacion->nodeValue = $lcObligacion;
			$loObligacionesEmisor->appendChild($loCodigoObligacion);
			unset($loCodigoObligacion);
		}
		$loEmisor->appendChild($loObligacionesEmisor);
		unset($loObligacionesEmisor);

		$loDireccionFiscal = $this->oDomDoc->createElement('DireccionFiscal');
		foreach ($this->aCnf['direccionFiscal'] as $lcClave => $lcValor) {
			$loDireccionFiscal->setAttributeNode(new \DOMAttr($lcClave, $lcValor));
		}
		$loEmisor->appendChild($loDireccionFiscal);
		unset($loDireccionFiscal);

		foreach ($this->aCnf['tributo'] as $lcClave => $lcValor) {
			$loTributoEmisor = $this->oDomDoc->createElement('TributoEmisor');
				$loTributoEmisor->setAttributeNode(new \DOMAttr('CodigoTributo', $lcClave));
				$loTributoEmisor->setAttributeNode(new \DOMAttr('NombreTributo', $lcValor));
			$loEmisor->appendChild($loTributoEmisor);
			unset($loTributoEmisor);
		}

		if (count($this->aCnf['contacto'])>0) {
			$loContacto = $this->oDomDoc->createElement('Contacto');
			foreach ($this->aCnf['contacto'] as $lcClave => $lcValor) {
				$loContacto->setAttributeNode(new \DOMAttr($lcClave, $lcValor));
			}
			$loEmisor->appendChild($loContacto);
			unset($loContacto);
		}
	...
	/*
	 *	Organiza la información del emisor para DS en un nodo
	 */
	public function datosEmisorDS()
	{
		// $loDomDoc = new \DOMDocument($this->aCnf['versionXml'], $this->aCnf['encodingXml']);
		$loEmisor = $this->oDomDoc->createElement('Cliente');

		// Genera nodo con datos del emisor
		foreach ($this->aCnf['emisorDS'] as $lcClave => $lcValor) {
			$loEmisor->setAttributeNode(new \DOMAttr($lcClave, $lcValor));
		}
		...
		$loObligacionesCliente = $this->oDomDoc->createElement('ObligacionesCliente');
		foreach ($this->aCnf['obligaciones'] as $lcObligacion=>$lcDescrOblig) {
			$loCodigoObligacion = $this->oDomDoc->createElement('CodigoObligacion');
			$loCodigoObligacion->nodeValue = $lcObligacion;
			$loObligacionesCliente->appendChild($loCodigoObligacion);
		...
		$loEmisor->appendChild($loObligacionesCliente);
		unset($loObligacionesCliente);

		foreach ($this->aCnf['tributo'] as $lcClave => $lcValor) {
			$loTributoCliente = $this->oDomDoc->createElement('TributoCliente');
				$loTributoCliente->setAttributeNode(new \DOMAttr('CodigoTributo', $lcClave));
				$loTributoCliente->setAttributeNode(new \DOMAttr('NombreTributo', $lcValor));
			$loEmisor->appendChild($loTributoCliente);
			unset($loTributoCliente);
		}
	 ...
	 */
	public function nodoPadreGenerico($tcNodoPadre, $tcNodoHijo, $taDatas)
	{
	 ...
	 */
	public function nodoGenerico($tcNodo, $taData)
	{
