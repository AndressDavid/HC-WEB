<?php
/* ********************  NOTAS A DOCUMENTO SOPORTE ADQUISICIONES  ******************** */
namespace NUCLEO;

require_once __DIR__ . '/class.FeConsultar.php';


class FeConsultarDSNC extends FeConsultar
{
	...
	private function consultaFactura($tcNumFac, $tcNumNota)
	{
		...
		$lcDocNota = str_pad($tcNumNota,6,'0',STR_PAD_LEFT);

		// Consultar fecha de Nota
		$laFemov = $this->oDb
			->select('IDTR, FECC, USRC')
		...
		if ( $this->oDb->numRows()==0 ) {
			$this->aError = [
				'Num' => '010',
				'Dsc' => "No se encontró el documento tipo {$this->cTipoFac} número {$tcNumNota} de la factura {$tcNumFac} en FEMOV.",
			];
			return;
		}

		// Consulta movimiento de Nota
		$lnFechaFac = strval($laFemov['FECC']); // cuando hacen nota con fecha del mes anterior ¿qué se debe hacer??
		$lcNumFac = ltrim(trim($laFemov['IDTR']),'0');
		$lnAnio = substr($lnFechaFac, 0, 4);
		$lnMes =  substr($lnFechaFac, 4, 2);
		$lnDia =  substr($lnFechaFac, 6, 2);

		$laMovsFac = $this->oDb
			->select('MOVSEC, MOVCTA, MOVAUX, MOVVRL, MOVCMP, MOVFCH, MOVHOR, MOVLOC, MOVCAN, MOVUSR, MOVAÑO||MOVMES||MOVDIA FECEMI')
			...
			->from('CTBMOV')
			->where([ 'MOVSUC'=>$lcSucursal, 'MOVAÑO'=>$lnAnio, 'MOVMES'=>$lnMes, 'MOVCBT'=>$lcTipoCbt, 'MOVPAQ'=>'ANU', 'MOVDOC'=>$lcDocNota ])
		...
		if ( $this->oDb->numRows()==0 ) {
			$this->aError = [
				'Num' => '010',
				'Dsc' => "No se encontró movimiento del documento tipo {$this->cTipoFac} número {$tcNumNota}.",
			];
			return;
		}
		...
		// CUFE Documento Soporte Anulado
		$lcNumDS = substr($lcNumFac, 2);
		$laQuery = $this->oDb
			->select('CUFE, FECC')
			->from('FEMOV')
			->where([ 'FACT'=>$lcNumDS, 'TIPR'=>'DSFA' ])
			->get('array');
		if ( $this->oDb->numRows()>0 ) {
			$this->aFactura['CUDS'] = trim($laQuery['CUFE']);
			$this->aFactura['FechaFac'] = trim($laQuery['FECC']);
		}
		...
		foreach ($laMovsFac as $lnKey => $laMov) {
			...
			switch (true) {
				...
				case in_array($lcCta4, ['2205','2335']) && $laMov['MOVCTA']!=='23352502':
					...
					$this->aFactura += [
						'DocContab' => $lcDocNota,
						'Fecha' => $laMov['FECEMI'], // $laMov['MOVFCH'],
				...
				case in_array($lcCta4, ['2365','2368']):
					...
					$laRetenc = [
						...
						'porcentaje' => round(floatval(str_replace(',','.',trim($laMov['MOVCMP'])))*($lcCta4=='2368'?10:1),4),
					];
					$lbEsNuevo = true;
	...
	public function crearArrayFac()
	{
		...
		if ($lbPrueba) {
			...
			$this->aFactura['Factura'] = intval(str_replace($this->aCnf['resDS']['Producción']['PrefijoNumeracion'], '', $this->aFactura['Factura'])) + $this->aCnf['resDS']['Pruebas']['ConsecutivoInicial'];
			$this->aFactura['Factura'] = $this->aCnf['resDS']['Pruebas']['PrefijoNumeracion'] . $this->aFactura['Factura'];
		}
		...
		// Contingencia
		$lbNoEsContingencia = $this->cTipDocXml=='NS';
		...
		/********************* Cabecera *********************/
		$laFactura['Documento'] = [
			'Prefijo' => $this->aCnf['tipoDoc']['NS']['prefijo'],
			'NumDoc' => intval($this->aFactura['Numero']),
		];
		...
		$laData = [
			...
			'TipoNota' => $this->aCnf['tipoDoc'][$this->cTipDocXml]['tipoFac'],
			'Observaciones' => '',
		...
		/********************* MotivosNota *********************/
		$laFactura['MotivosNota'] = [
			[
				// 'ID' => '1',
				'ConceptoCorreccion' => 2,
				'DescripcionConcepto' => 'Anulación del documento soporte en adquisiciones efectuadas a sujetos no obligados a expedir factura de venta o documento equivalente',
			]
		];

		/********************* SoporteAdquisicionesRelacionados *********************/
		$laFactura['SoporteAdquisicionesRelacionados'] = [
			[
				'Numero' => $this->aFactura['Factura'],
				'CUDS' => $this->aFactura['CUDS'],
				'FechaEmision' => FeFunciones::formatFecha($this->aFactura['FechaFac']),
			]
		];
...
//		];
		...
		if (count($this->aDescCargos)>0) {
			...
			foreach ($this->aDescCargos as $laDescCargo) {
				...
				$laDescCargos[] = [
					...
					'Porcentaje' => $lnBruto==0 ? '' : number_format((100*$laDescCargo['Valor']/$lnBruto),2,'.',''),
				];
			}
		...
		$lnBaseImponible = 0; // Base imponible en ceros si no hay impuestos, para que pase en la DIAN
		if ($lcTipoRegRete!=='NO') {
			$laData = [];
			$laRetenciones = [
			...
			foreach($laRetenciones as $lcReteKey=>$laReteConfig){
				...
				foreach($this->aFactura[$lcReteKey] as $laReteData){
					...
					//if (in_array($laReteConfig[3], ['05','06'])) {
					if (in_array($laReteConfig[3], ['01'])) {
		...
		$laData = [
			...
			'basei' => number_format($lnBaseImponible,2,'.',''), // number_format($lnBruto,2,'.',''),
			'impst' => '0',
...
//	*********************************************************************
		...
		foreach ($this->aDetalles as $laDet) {
			...
			$lcVrTotal = number_format(abs($laDet['Valor']),2,'.','');
			$lcCantidad = 1;
			$lcVrUnidad = number_format(abs($laDet['Valor']) / $lcCantidad,2,'.','');
			...
			$laData = [
				...
				'vrud' => $lcVrUnidad,
				'vrtt' => $lcVrTotal,
				'fecc' => $lcFechaEmision, // $this->aFactura['FechaFac'],
			...
			if ($lbHayRetenciones && in_array($lcTipoRegRete, ['PORLINEA','TODO'])) {
				...
				foreach($laRetenciones as $lcReteKey=>$laReteConfig){
					if ($this->bCalcularPorLinea[$lcReteKey]) { // Problema los porcentajes
						$lnBaseImpRet = round($lnBaseImponible * $laDet['Valor'] / $lnBruto);
						$laData['rete'][] = [
						...
						if ($lnBaseImp<$lnBaseImpRet) {
							$lnBaseImp = $lnBaseImpRet;
						}
					} else {
