<?php
	...
	$lcBody=$lcBodyAyer="";
	$llValores=false;
	$lnRegistrosAyer=$lnConsumosAyer=$lnIngresosAyer=$lnRegistros=$lnConsumos=0;
	$laIngresos = array();
	...
	if(is_array($laConsumosAyer)==true){
		for($lnItem=0;$lnItem<count($laConsumosAyer)-1;$lnItem++){
			...
			$lcPicture=($lnConsumos==0?'bullseye':($lnConsumos==$laConsumoAyer['DATO03']?'equals':($lnConsumos>$laConsumoAyer['DATO03']?'angle-double-down':'angle-double-up')));
	...
	$lnConsumosAyer=intval($lnConsumosAyer);
	$laCampos = [
				'A.INGCSF AS INGRESO', 
				'A.FACCSF AS FACTURA',
	...
	$laConsumos = $goDb->select($laCampos)
					  ...
					  ->getAll("array");
	$lnConsumos = 0;
	$lnIngresoMaximo=intval(trim($goDb->obtenerTabmae1('DE2TMA', 'PARFACT', "CL1TMA='INGPFAC' AND ESTTMA=''", null, '')));
	$lcAlertar='';
	...
	if(is_array($laConsumos)==true){
		foreach($laConsumos as $laConsumo){
			...
			$lcAlertar=empty($lcAlertar) ? (intval($laConsumo['INGRESO'])>$lnIngresoMaximo?'A':''):$lcAlertar;
			$lcBody.='<tr>';
	...
	$lnConsumos=intval($lnConsumos);
	$lcBg = ($lnConsumos>$lnConsumosAyer?"danger":"success");
	$lcPicture=($lnConsumos==0?'bullseye':($lnConsumos==$lnConsumosAyer?'equals':($lnConsumos>$lnConsumosAyer?'angle-double-up':'angle-double-down')));	
?><!doctype html>
					...
					<div class="card mt-3">
						...
						<div class="card-body">			
							<div class="card text-center">
								<table class="card-body table table-borderless table-sm text-white">
									<thead>
							...
							<div class="row">
								<div class="col-md-12">
									<p class="m-t-5 m-b-5">
										<a class="btn btn-<?php print($lcBg); ?> btn-sm btn-block" data-toggle="collapse" href="#consumosNoFacturados" role="button" aria-expanded="false" aria-controls="consumosNoFacturados">Ver detalle  <b><?php print($ltAhora->format("Y-m-d")); ?></b></a>
									</p>
								</div>
							</div>
							<div class="collapse" id="consumosNoFacturados">
								<div class="card text-left">
									<table class="card-body table table-dark table-sm table-striped table-hover">
										<thead>
											<tr>
												<th>Ingreso</th>
												<th>Fecha ingreso</th>
