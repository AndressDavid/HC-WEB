...
class SmbClient
{
	...
	/**
	 * Verifica si existe un archivo o una carpeta
	 * Usa la función dir para obtener el resultado
	 *
	 * @param string $remote_path ruta dentro de la cual se buscará
	 * @param string $remote_filename nombre de archivo o carpeta a buscar
	 * @return string file o dir si existe, false si no existe
	 */
    public function file_exists ($remote_path = '', $remote_filename = '')
    {
		$lbReturn = false;
		if (!empty($remote_path) && !empty($remote_filename))
		{
			$laDir = $this->dir($remote_path, $remote_filename);
			if (isset($laDir[0]['isdir'])) {
				$lbReturn = $laDir[0]['isdir'] ? 'dir' : 'file';
			}
		}
    ...
    private function build_full_cmd ($cmd = '')
    {
		...
		// se adiciona validación en caso que no exista usuario
