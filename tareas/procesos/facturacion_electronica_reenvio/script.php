...
*/
	...
	function tareaProgramaPrincipal(){
		...
		try {
			$lnIntervaloHoras = 1;
			$lnHorasEnvio = 72;
		//	$lcWhere = "ESTA IN ('01','04') AND (FECC * 1000000 + HORC BETWEEN REPLACE(REPLACE(SUBSTR(CHAR(NOW() - {$lnHorasEnvio} HOURS), 0, 20), '.', '') ,'-', '') AND REPLACE(REPLACE(SUBSTR(CHAR(NOW() - {$lnIntervaloHoras} HOURS), 0, 20), '.', ''), '-', ''))";
		//	$lcWhere = "ESTA IN ('01','04') AND FECC = REPLACE(SUBSTR(CHAR(NOW()), 1, 10), '-', '') AND (FECC * 1000000 + HORC < REPLACE(REPLACE(SUBSTR(CHAR(NOW() - {$lnIntervaloHoras} HOURS), 0, 20), '.', ''), '-', ''))";
