...
class FeConsultar25NC extends FeConsultar
{
	...
	private function consultaFactura($tcNumFac, $tcNumNota)
	{
		...
		// Consulta movimiento de Nota Crédito
		...
		$laMovsFac = $this->oDb
			...
			->where([ 'M.MOVSUC'=>$lcSucursal, 'F.CBTNC'=>$lcTipoCbt, 'F.PAQNC'=>'ANU', 'F.FACNC'=>$tcNumFac, 'F.NOTNC'=>$tcNumNota ])
			->getAll('array');
		...
		foreach ($laMovsFac as $lnKey => $laMov) {
			...
			switch ($lcTipoCta) {
				...
				/********** Cabecera de Nota Crédito **********/
				case '13':

					$lcFacAnul = ((int)substr($laMov['MOVAUX'],14,8)).'';
					$this->aFactura += [
						'Numero' => $tcNumNota,
						'Factura' => $tcNumFac,
						'DocContab' => $lcDocFac,
						...
						'Descripcion' => '',
						'DescripNota' => '',
						'DescripAnul' => 'Anulación de Factura No.'.$lcFacAnul,
					...
					$laQuery = $this->oDb
						->select('CUFE,FECC')
						->from('FEMOV')
						->where(['FACT'=>$lcFacAnul, 'TIPR'=>'25FA'])
						->get('array');
					if ($this->oDb->numRows()>0) {
						$this->aFactura['CUFE'] = trim($laQuery['CUFE']);
						// Fecha factura anulada
						$this->oDb->where(['MOVAÑO'=>substr($laQuery['FECC'],0,4), 'MOVMES'=>substr($laQuery['FECC'],4,2)]);
					}
					...
					$laQuery = $this->oDb
						->select('DETDET')
						->from('CTBDET')
						->where([ 'DETSUC'=>$lcSucursal, 'DETAÑO'=>$lnAnio, 'DETMES'=>$lnMes, 'DETCBT'=>$lcTipoCbt, 'DETDOC'=>substr(str_pad($tcNumNota,6,'0',STR_PAD_LEFT),0,6) ]) // 'DETPAQ'=>'ANUL'
						->getAll('array');
					if ($this->oDb->numRows()>0) {
						foreach ($laQuery as $laDetalle) {
					...
					if ( $this->oDb->numRows()>0 ) {
						...
						if (isset($this->aCnf['cliente']['formaPagoDoc']['25NC'])) {
	...
	public function crearArrayFac()
	{
		...
		// ];
		// if ($this->bEsFacElectr) $laData['fact'] = $this->aCnf['resFactura'][$this->aCnf['ambiente']]['PrefijoNumeracion'].$this->aFactura['FacAnulada'];
