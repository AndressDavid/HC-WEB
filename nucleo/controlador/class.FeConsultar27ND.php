...
class FeConsultar27ND extends FeConsultar
{
	...
	public function crearArrayDatos($tnFactura=0, $tnNota=0, $tnDocAdj=0)
	{
		...
		$this->consultaCabecera($tnFactura, $tnNota, $tnDocAdj);
	 ...
	 */
	private function consultaCabecera($tcNumFac, $tcNumNota, $tcNumAnx)
	{
		// Consulta cabecera de Nota
		$this->aFactura = $this->oDb
			->select('F.FRAGLC, F.FEFGLC, F.NOTGLC, F.CONGLC, F.NITGLC, F.PLNGLC, F.VLRACC, F.CONCGLC, X.OBSEGLC, X.FMOGLC, X.HMOGLC, X.USRGLC')
			...
			->select('P.NM1PAC, P.NM2PAC, P.AP1PAC, P.AP2PAC, P.DR1PAC, P.DR2PAC, P.TELPAC, P.MAIPAC, P.FNAPAC, P.SEXPAC, P.NHCPAC, T.FEFCAB, T.VAFCAB, IFNULL(M.CUFE,\'\') CUFE, N.USRC')
			...
			->tabla('AMGLCAB F')
			->innerJoin('AMGLCAB AS X', 'F.FRAGLC=X.FRAGLC AND F.NOTGLC=X.NOTGLC AND X.CONGLC>1000', null)
			...
			->leftJoin( 'FEMOV   AS N', 'F.FRAGLC=N.FACT AND N.DOCA=F.NOTGLC AND N.TIPR=\'27ND\'', null)
			...
			->leftJoin( 'RIARGMN AS U', 'N.USRC=U.USUARI', null)
			->where([ 'F.FRAGLC' => $tcNumFac, 'F.NOTGLC' => $tcNumAnx ])
			->where('F.CONGLC', '<', 1000)
			->get('array');
		...
		$this->aFactura['NOTDEB'] = $tcNumNota;
	...
	public function crearArrayFac()
	{
		...
		if ($lbPrueba) {
			...
			$this->aFactura['NOTDEB'] = strval($this->aFactura['NOTDEB'] + $this->aCnf['facture']['Pruebas']['sumarnota']);
		...
		$lcObservaciones = 'Me permito enviar a ustedes respuesta de su objeción sobre la Factura No. ' . $this->aFactura['FRAGLC'] .
							...
							$this->cSLD . $this->aFactura['OBSEGLC'] . // '|Anula la NC' . $this->aFactura['NOTGLC']
		...
		$laFactura['Documento'] = [
			...
			'NumDoc' => $this->aFactura['NOTDEB'],
		];
		$laData = [
			'Prefijo' => $this->aCnf['tipoDoc'][$this->cTipDocXml]['prefijo'],
			'Numero' => $this->aCnf['tipoDoc'][$this->cTipDocXml]['prefijo'].$this->aFactura['NOTDEB'],
...
// OJO ES OBLIGATORIO SI ES FACTURA EN CONTINGENCIA
		...
		// 	'UrlAdjunto' => '',
		// ];
		// $laFactura['DocumentosAdicionalesReferencia'] = $this->nodoGenerico('DocumentosAdicionalesReferencia', $laData);
		...
		foreach ($this->aDetalles as $laDet) {
			...
			$laFactura['Lineas'][$lnCns] = [
				...
				'cadd' => [
					'conceptoglosa'	=> empty($laDet['dscGlosa']) ? ' ' : $laDet['dscGlosa'],
		...
		$laFactura['Extensiones'] = [
			...
			// Nota
				...
				'elaboradopor' => $this->aFactura['USRC'],
