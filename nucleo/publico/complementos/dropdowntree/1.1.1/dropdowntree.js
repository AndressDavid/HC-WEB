...
var dropDownOptions = {
    ...
    rtl: false,
	uncheckIcon: 'fa-square',
	checkIcon: 'fa-check-square',
};
...
$.fn.DropDownTree = function(options){
    ...
    function RenderData(data, element){
        for(var i = 0 ; i< data.length ; i++){
            ...
            if(!element.is("li")){
                element.append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="far '+options.uncheckIcon+' select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
            }else{
                element.find("ul").append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="far '+options.uncheckIcon+' select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
            }
    ...
    $(options.element).on("click",".select-box",function(e){
        e.stopPropagation();
        var checked;
        if($(this).hasClass(options.uncheckIcon)){
            //will select
            checked=true;
            $(this).removeClass(options.uncheckIcon).addClass(options.checkIcon);
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").removeClass(options.uncheckIcon);
                $(this).parents("li").first().find(".select-box").addClass(options.checkIcon);
            }
        }else{
            //will unselect
            checked=false;
            $(this).removeClass(options.checkIcon).addClass(options.uncheckIcon);
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").addClass(options.uncheckIcon);
                $(this).parents("li").first().find(".select-box").removeClass(options.checkIcon);
                $(this).parents("li").each(function(){
                    $(this).find(".select-box").first().removeClass(options.checkIcon);
                    $(this).find(".select-box").first().addClass(options.uncheckIcon);
                });
            }
        }
    ...
    $(options.element).append('<button class="btn btn-default dropdown-toggle form-control form-control-sm" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><span class="dropdowntree-name">'+options.title+'</span></button>');
    ...
    $(options.element).init.prototype.GetSelected = function(title){
        ...
        $(this).find("."+options.checkIcon).each(function(){
