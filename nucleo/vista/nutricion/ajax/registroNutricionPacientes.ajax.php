		...
		if ($_SESSION[HCW_NAME]->oUsuario->getSesionActiva()==true){
			if(isset($_GET['cabecera'])==true){
				...
				if(isset($_POST['cEstado'])==true){
					$loNutriciones->setEstado(intval($_POST['cEstado']));
					$lnConsecutivo = ($loNutriciones->insertar($_SESSION[HCW_NAME]->oUsuario->getUsuario())==true?$loNutriciones->getConsecutivo():0);
						
					if($loNutriciones->getConsecutivo()>0 && !empty($_POST['cObservacion'])){
						$loNutriciones->insertarObservacion(0, $_POST['cObservacion'], $_SESSION[HCW_NAME]->oUsuario->getUsuario());
					}
				}
				print($lnConsecutivo);
				...
				if(isset($_POST['cProveedorCodigo'])==true && isset($_POST['cProveedorNombre'])==true){
					$loNutriciones->insertarProveedor(intval($_POST['cProveedorCodigo']), $_POST['cProveedorNombre'],$_SESSION[HCW_NAME]->oUsuario->getUsuario());
				}
				
				include (__DIR__ .'/../../../publico/headJSON.php');
				print(json_encode($loNutriciones->getProveedores()??''));		
				
			}else if(isset($_GET['proveedorEspecifico'])==true){
				$laProveedor = array();
				if(isset($_POST['PROVEEDOR'])==true){
					$laProveedor = $loNutriciones->buscarProveedor(intval($_POST['PROVEEDOR']));
				}
